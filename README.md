...
{\it i)} For the diversity of spatial patterns, a novel perceptron of moment-residual-coherent features (named MRCF) is proposed to make a discriminative representation of moment-residual-coherent patterns that have been extracted and enriched from depthwise-based tensors.
{\it ii)} To adapt to the channel-elasticity moments of MRCF in a shallow backbone, two novel adaptive residual mechanisms are presented: an increase-moment residual is based on the expanding flexibility of a pointwise operator, while the decrease-moment one is on the aggregated spatial patterns of a fused tensor.
...
{\it iii)} Finally, a lightweight network is introduced by addressing three robust caret-shape segments of MRCF blocks that allow the learning process to effectively capture the moment-residual-coherent patterns of a given tensor.
